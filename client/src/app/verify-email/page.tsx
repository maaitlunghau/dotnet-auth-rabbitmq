      ...
      <div className="w-full max-w-md animate-fade-in" style={{ animationDelay: "0.1s" }}>
        ...
        {status === "success" ? (
          <div className="bg-card border border-border-base rounded-xl p-8 text-center animate-scale-in">
            ...
            <h2 className="text-xl font-semibold text-content-primary mb-2">Email Verified!</h2>
            <p className="text-sm text-content-secondary mb-4">
              Your account has been activated successfully.
            </p>
            <p className="text-xs text-content-tertiary">Redirecting to login...</p>
          </div>
        ) : (
          <>
            ...
            <div className="bg-card border border-border-base rounded-xl p-6">
              {/* Error Message */}
              {status === "error" && (
                <div className="mb-4 p-3 text-sm text-danger bg-danger-bg rounded-lg border border-danger/20 flex items-center gap-2 animate-shake">
                  <XCircle size={16} />
                  {error}
                </div>
              )}
                ...
                {code.map((digit, i) => (
                  <input
                    ...
                    className={`w-10 h-12 text-center text-lg font-semibold rounded-lg border
                      bg-input text-content-primary
                      focus:outline-none focus:ring-2 transition-all
                      ${status === "error"
                        ? "border-danger focus:ring-danger/30"
                        : "border-border-base focus:ring-border-active focus:border-border-active"
                      }
                      ${digit ? "border-brand/50" : ""}
                    `}
              ...
              {verifyMutation.isPending && (
                <div className="flex items-center justify-center gap-2 text-sm text-content-secondary mb-4">
